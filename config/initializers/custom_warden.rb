  ...
  class Proxy
    ...
    def authenticate!(*args)
      ...
      unless user.is_authenticated
        raise Warden::Unauthorized.new(code: 'COC009', message: 'This account has not been authenticated by email.')
      end

      unless user.is_active
        raise Warden::Unauthorized.new(code: 'COC010', message: 'This account has been deactivated.')
      end

      if user.withdrawaled_at.present?
        raise Warden::Unauthorized.new(code: 'COC011', message: 'This account is in the process of withdrawal from membership.')
      end
